...
def create_plane(location: Tuple[float, float, float] = (0.0, 0.0, 0.0),
    ...
    # Set the name of the object if provided
    if name is not None:
        current_object.name = name
    ...
    # Set the material's transparency settings
    bsdf = material.node_tree.nodes.get("Principled BSDF")
    bsdf.inputs['Alpha'].default_value = 0  # Set alpha to 0 for full transparency
    bsdf.inputs['Base Color'].default_value = (1, 1, 1, 0)  # White color with 0 alpha

    # Assign the material to the plane
    if current_object.data.materials:
        current_object.data.materials[0] = material
    else:
        current_object.data.materials.append(material)

    # Enable transparency
    current_object.active_material.blend_method = 'BLEND'  # Use 'BLEND' for transparent materials
    current_object.active_material.shadow_method = 'NONE'   # Disable shadows
